...
public class QuValidateId {
	...
	public static void main(String[] args) {
		...
		toCharArray() 메서드를 사용하면 된다. 해당 메서드는 String
		을 char타입의 배열로 변환해준다. */
	...
	static boolean idValidate(String inputld) {
		
		//아이디 유효성 검증 확인용 변수
		boolean idOk = true;
		
		//아이디의 길이가 8~12인지 검증
		if(inputld.length()>=8 && inputld.length()<=12) {
			//입력한 아이디의 길이만큼 반복해서
			for(int i=0 ; i<inputld.length() ; i++) {
				
				//각 문자가 영문 또는 숫자인지 확인한다.
				char idChr = inputld.charAt(i);
